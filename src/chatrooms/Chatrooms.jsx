  ...
  const fetchChatData = async () => {
    try {
      ...
      if (chatroomSnap.exists()) {
        ...
        setUsername(
          currentUser.uid === senderId ? sender?.username : receiver?.username
        );
      } else {
  ...
  return (
    <div className="min-w-[100vw] min-h-[100vh] overflow-hidden">
      <div className="w-full h-[100vh] flex items-center justify-center ">
        <div
          className="bg-black p-6 shadow-lg w-full
        ...
        >
          <nav className="bg-white flex w-full justify-between p-3 shadow-sm rounded-md">
            <h1 className="font-bold tracking-wider capitalize">
              ChatRoom:{username}
            </h1>
            <button
          ...
          <div className="w-full h-[70vh] flex flex-col items-start gap-3 overflow-auto ">
            ...
            {messages.map((data, i) => (
              <div
                key={i}
                className={`message  text-green-400 ${
                  data.senderId === currentUser.uid ? "text-end" : "text-start"
                } flex `}
                style={{
                  alignSelf:
                    data.senderId === currentUser.uid
                      ...
                      : "flex-start",
                  backgroundColor:
                    data.senderId === currentUser.uid ? "#DCF8C6" : "#FFFFFF",
                  padding: "5px",
                  borderRadius: "10px",
                  maxWidth: "80%",
                }}
              >
                <div className="flex flex-col w-40 md:w-60">
                  <span className="text-start">{data.message}</span>
                  <div className="flex justify-between ml-1 mr-1">
                    ...
                    {data.senderId === currentUser?.uid && (
                      <button
                        onClick={() => handleDeleteChat(data.id)}
                        className=" "
                      >
                        <svg
                          class="bin-top"
                        ...
                        <svg
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <mask id="path-1-inside-1_8_19" fill="red">
          ...
          <div className="flex gap-2 w-full h-8   sticky">
            <form onSubmit={handleSendMessage} className="flex   w-full">
              <div className="form-control flex md:gap-3 gap-2">
                <input
                  class="input input-alt text-sm   placeholder:text-sm md:placeholder:text-xl w-full  "
                ...
                />
                <span className="input-border h-2  input-border-alt"></span>
              </div>
              ...
              {emoji && (
                <div className="fixed bottom-5 right-5 md:bottom-12 md:right-10 z-10">
                  <div className="h-[250px] overflow-auto md:h-[300px]">
                    <Picker
                      data={data}
