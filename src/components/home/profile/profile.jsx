   ...
   <section className='size flex gap-[4rem] relative'>
    <div className='mt-[9rem] flex-[2]'>
        <div className='flex items-end gap-4'>
          <h2 className='md:text-3xl lg:text-2xl text-xl sm:text-5xl font-bold capitalize'> {getUserData?.username} </h2>
        ...
        <div className=' flex items-center gap-4 lg:gap-4 md:gap-5 mt-[1rem]  border-b border-gray-500 mb-[3rem]'>
        {activities.map((prof, i) => (
  <div key={i} className={` text-sm sm:text-2xl font-title ${prof.title === currentActivity.title 
 ...
 <Model modal={modal} setModal={setModal} >
    ...
    top-0 w-[18rem] md:relative ${modal ? "translate-x-0" : "translate-x-[100%] md:translate-x-0"}
     ...
     <div className='sticky top-7 flex flex-col justify-between '>
        <img className='w-[3.5rem] h-[3.5rem] object-cover border rounded-full' 
        src={getUserData?.image || "/profile.jpg"} alt="" />
