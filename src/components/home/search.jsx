...
const Search = ({ modal, setModal }) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      setShowSearch(currentScrollY < lastScrollY || currentScrollY < 10); 
  ...
  return (
    <>
      {modal && showSearch && (
        <div className="flex items-center gap-1 bg-gray-200 px-2 py-1 rounded-md relative z-30 w-full transition-all duration-300">
          ...
          />
          {search !== "" && (
            <div className='absolute right-0 left-0 top-full bg-white shadow rounded-md z-50'>
              {searchData.length > 0 ? (
